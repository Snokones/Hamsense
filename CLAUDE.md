...
- **`lite-ui`** (this branch) — same firmware as `main`, but `web_pages.h` is the condensed mobile-first page: no map, no theme picker, no 7-day panel, **no ZIP box and no forecast**, and zero external CDN dependencies so it loads on a phone with no internet. Coordinates and elevation come from a collapsed LOCATION box writing into the `STATION` object (the constant is the built-in default). **Keep this page minimal** — the location box is setup, not a reading; new display panels belong on `main`.
...
- **Location**: the `STATION` object in `web_pages.h` (`{lat, lon, elevM}`) is the only source of position. It drives sunrise/sunset and the pressure correction; the firmware knows nothing about it. Set from the LOCATION box, persisted in localStorage, falling back to the compiled-in constant. Elevation auto-fills from Open-Meteo but **the lookup must stay optional** — this page has to work fully offline on the `PotaSense` AP, so nothing may block on it.
...
- **Power**: the node serves an always-on page, so it never sleeps. `setCpuFrequencyMhz(80)` is the floor with WiFi active. **Never add deep or light sleep to the base** — it would kill the web server and the USB CDC port used for flashing. Leave `WiFi.setSleep()` at its default; the satellite already retries 8× to ride through power-save windows.
